...
bson_data * bson_empty(bson_data * obj){
    static const char * data = "\005\0\0\0\0";
    return bson_init(obj, (char *)data, 0);
}

void bson_copy(bson_data* out, const bson_data* in){
    if (!out) return;
...
bson_buffer * bson_append_bson( bson_buffer * b , const char * name , bson_type bt, const bson_data* bson_data){
    if ( ! bson_append_estart( b , bt , name , bson_size(bson_data) ) ) return 0;
    bson_append( b , bson_data->data , bson_size(bson_data) );
    return b;
}
