...
void MainWindow::saveRemotefile(RemoteFile* file,bool deleteOnFailure) {
	if (!file->save()) {
		statusBar()->showMessage(i18n("Error: Remote file %1 NOT saved!", file->getFilename()));
...
void MainWindow::saveAppfile(ApplicationProfileFile* file,bool deleteOnFailure) {
	if (!file->save()) {
		statusBar()->showMessage(i18n("Error: Application Profile %1 NOT saved!", file->getFilename()));
		if (deleteOnFailure) delete file;
	} else {
		/* update the model */
		ap_model->edit(file);
...
void MainWindow::saveDesktopfile(DesktopProfileFile* file,bool deleteOnFailure) {
	if (!file->save()) {
		statusBar()->showMessage(i18n("Error: Desktop Profile %1 NOT saved!", file->getFilename()));
		if (deleteOnFailure) delete file;
	} else {
		/* update the model */
		dp_model->edit(file);
...
qDebug() << remotefile->getFilename();
	...
	} else dp_clicked(ui->remoteslistView->currentIndex());
...
void MainWindow::btnDPRemove() {
	...
	} else dp_clicked(ui->dektoplistView->currentIndex());
