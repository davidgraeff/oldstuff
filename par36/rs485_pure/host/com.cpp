...
void printFoundAnswer(int r, char* input) {
	for (int i=0;i<r;++i) {
...
void printAnswer(int r, char* input) {
	...
	if ((unsigned int)input[3] < sizeof(cmdToString))
		printf("From: %u, To: %u , CMD: %s, Value: %u",  (uint8_t)input[2] >> 4, (uint8_t)input[2] & 0x0f, cmdToString[(unsigned int)input[3]], (uint8_t)input[4]);
	printf("\n");
}
...
int main(int argc, char** argv) {
	...
	const int baud = 9600; //115200
	SerialPort sp(argv[1], baud, 0, 8, 1, false);
	if (!sp.isOpen()) {
		return -1;
	}
	//     uint8_t v1,v2;
...
// 			}
			...
			{
				printf("Scan lights:\n");
				unsigned char t[6] = {0xff, 0xff, 0, CMD_INITIALIZE, 1, 0x00};
				for (int i=1;i<16;++i) {
					t[2] = i & 0x0f;
					sp.writeData((char*)t,sizeof(t));
					usleep(1000*250); // 400 ms
					int r = sp.readData(input, sizeof(input));
					if (r<=0) {
						std::cout << "err" << i << ".."<< std::flush;
					} else
						printAnswer(r, input);
				}
				printf("\n");
