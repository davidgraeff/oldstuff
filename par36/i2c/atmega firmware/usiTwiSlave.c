...
volatile uint8_t txbuffer[16];			// Transmission buffer to be read from the master
volatile uint8_t buffer_pos = 0;
volatile uint8_t has_packet = 0;
struct uartPacket tempBuffer, finalBuffer;
		...
		case USI_SLAVE_CHECK_ADDRESS:
			if (USIDR == 0 || (USIDR & ~1) == slaveAddress)     // If adress is either 0 or own address		
				...
				if (  USIDR & 0x01 )
					...
					buffer_pos=0; // Buffer position = 0
					}
				else
					{
...
//###################################### Master Write Data Mode - Slave transmit
		...
		case USI_SLAVE_SEND_DATA:
			USIDR = txbuffer[buffer_pos]; 	// Send data byte
			
			++buffer_pos; 					// Increment buffer address for next byte
...
//######################################## Master Read Data Mode - Slave receive
		...
		case USI_SLAVE_GET_DATA_AND_SEND_ACK:
			...
			if (buffer_pos==0) {
				...
				buffer_pos = 0;
				tempBuffer.value = data;
				if (!has_packet) {
					finalBuffer.cmd = tempBuffer.cmd;
					finalBuffer.value = tempBuffer.value;
					has_packet = 1;
				}
			}
