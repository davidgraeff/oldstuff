...
public class SceneModel {
	...
	private void changeWithoutCat(JSONObject json) throws JSONException {
		String entryid = json.getString(m_key);
	...
	public void change(JSONObject json) throws JSONException {
		if (!json.has(m_key))
			return;
		...
		for (int i = 0; i < categoriesJSON.length(); ++i) {
			String catid = categoriesJSON.getString(i);
	...
	public void remove(JSONObject json) throws JSONException {
		if (!json.has(m_key))
			return;

		if (remove(json.getString(m_key))) {
			for (ModelChangeListener o : mChangeListener) {
				o.onModelDataChange();
			}
		}
